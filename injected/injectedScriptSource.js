(() => {
    ...
    function unexpected() {
      return new InvalidSelectorError(
        `Unexpected token "${tokens[
          pos
        ].toSource()}" while parsing css selector "${selector}". Did you mean to CSS.escape it?`
      );
    }
  ...
  var kNestedSelectorNames = new Set([
    "internal:has",
    "internal:has-not",
    "internal:and",
    "internal:or",
      ...
      switch (kind) {
        case "default":
          ...
          if (options.hasNotText !== void 0)
            return `locator(${this.quote(body)}, { hasNotText: ${this.toHasText(
              options.hasNotText
            )} })`;
  ...
  var PythonLocatorFactory = class {
    generateLocator(base, kind, body, options = {}) {
      switch (kind) {
        case "default":
          ...
          if (options.hasNotText !== void 0)
            return `locator(${this.quote(body)}, has_not_text=${this.toHasText(
              options.hasNotText
            )})`;
          return `locator(${this.quote(body)})`;
        case "frame-locator":
    ...
    regexToString(body) {
      ...
      return `re.compile(r"${normalizeEscapedRegexQuotes(body.source)
        .replace(/\\\//, "/")
        .replace(/"/g, '\\"')}"${suffix})`;
    }
    toCallWithExact(method, body, exact) {
        ...
        case "default":
          if (options.hasText !== void 0)
            return `locator(${this.quote(
              body
            )}, new ${clazz}.LocatorOptions().setHasText(${this.toHasText(options.hasText)}))`;
          if (options.hasNotText !== void 0)
            return `locator(${this.quote(
              body
            )}, new ${clazz}.LocatorOptions().setHasNotText(${this.toHasText(
              options.hasNotText
            )}))`;
          return `locator(${this.quote(body)})`;
        case "frame-locator":
    ...
    generateLocator(base, kind, body, options = {}) {
      switch (kind) {
        case "default":
          ...
          if (options.hasNotText !== void 0)
            return `Locator(${this.quote(body)}, new() { ${this.toHasNotText(
              options.hasNotText
            )} })`;
          return `Locator(${this.quote(body)})`;
        case "frame-locator":
  ...
  function matchesNode(node, template, isDeepEqual) {
    ...
    if (!matchesText(node.props.url, (_a = template.props) == null ? void 0 : _a.url)) return false;
  ...
  function renderAriaTree(ariaSnapshot, options) {
    const lines = [];
    const includeText =
      (options == null ? void 0 : options.mode) === "regex" ? textContributesInfo : () => true;
    const renderString =
      (options == null ? void 0 : options.mode) === "regex"
    ...
    const visit = (ariaNode2, parentAriaNode, indent) => {
      ...
      if (options == null ? void 0 : options.ref) {
        ...
        if (id) key += ` [ref=s${ariaSnapshot.generation}e${id}]`;
      }
      ...
      if (!ariaNode2.children.length && !hasProps) {
        ...
        ariaNode2.children.length === 1 &&
        typeof ariaNode2.children[0] === "string" &&
        !hasProps
      ) {
        const text = includeText(ariaNode2, ariaNode2.children[0])
          ? renderString(ariaNode2.children[0])
          : null;
  ...
  function getComponentKey(reactElement) {
    var _a, _b;
    return (_b = reactElement.key) != null
      ? _b
      : (_a = reactElement._currentElement) == null
      ? void 0
      : _a.key;
  }
  ...
  function buildNoTextCandidates(injectedScript, element, options) {
    ...
    if (element.nodeName === "IFRAME") {
      for (const attribute of ["name", "title"]) {
        if (element.getAttribute(attribute))
          candidates.push({
            ...
            )}[${attribute}=${quoteCSSAttributeValue(element.getAttribute(attribute))}]`,
      ...
      if (element.getAttribute(options.testIdAttributeName))
        candidates.push({
          engine: "css",
          selector: `[${options.testIdAttributeName}=${quoteCSSAttributeValue(
            element.getAttribute(options.testIdAttributeName)
          )}]`,
          score: kTestIdScore,
        });
    ...
    if (element.getAttribute(options.testIdAttributeName))
      candidates.push({
        ...
        selector: `[${options.testIdAttributeName}=${escapeForAttributeSelector(
          element.getAttribute(options.testIdAttributeName),
          true
        )}]`,
        score: kTestIdScore,
    ...
    if (
      ...
      candidates.push({
        engine: "css",
        selector: `${cssEscape(element.nodeName.toLowerCase())}[name=${quoteCSSAttributeValue(
          element.getAttribute("name")
        )}]`,
    ...
    if (
      ...
      if (element.getAttribute("type"))
        candidates.push({
          engine: "css",
          selector: `${cssEscape(element.nodeName.toLowerCase())}[type=${quoteCSSAttributeValue(
  ...
  function buildComponentsTreeVue3(instance2) {
    ...
    function getInstanceName(instance3) {
      ...
      if (name) return name;
      if (instance3.root === instance3) return "Root";
      for (const key in (_b = (_a = instance3.parent) == null ? void 0 : _a.type) == null
        ? void 0
        : _b.components) {
    ...
    function getRootElementsFromComponentInstance(instance3) {
      if (isFragment(instance3)) return getFragmentRootElements(instance3.subTree);
  ...
  function buildComponentsTreeVue2(instance2) {
    ...
    function getInternalInstanceChildren(instance3) {
      if (instance3.$children) return instance3.$children;
      if (Array.isArray(instance3.subTree.children))
        return instance3.subTree.children
  ...
  function filterComponentsTree2(treeNode, searchFn, result = []) {
    if (searchFn(treeNode)) result.push(treeNode);
    for (const child of treeNode.children) filterComponentsTree2(child, searchFn, result);
    return result;
  }
  ...
  var createVueEngine = () => ({
    queryAll(scope, selector) {
      ...
      const treeNodes = trees
        .map((tree) =>
          filterComponentsTree2(tree, (treeNode) => {
            if (name && treeNode.name !== name) return false;
  ...
  var InjectedScript = class {
    ...
    constructor(
      ...
      if (isUnderTest) this.window.__injectedScript = this;
    }
    eval(expression) {
    ...
    ariaSnapshot(node, options) {
      ...
      const generation =
        (((_a = this._lastAriaSnapshot) == null ? void 0 : _a.generation) || 0) + 1;
...
return module.exports.default();
    ...
    _createAriaIdEngine() {
      const queryAll = (root, selector) => {
        ...
        if (((_a = this._lastAriaSnapshot) == null ? void 0 : _a.generation) !== +generation)
          throw this.createStacklessError(
            `Stale aria-ref, expected s${
              (_b = this._lastAriaSnapshot) == null ? void 0 : _b.generation
            }e{number}, got ${selector}`
          );
        const result =
          (_d = (_c = this._lastAriaSnapshot) == null ? void 0 : _c.elements) == null
            ? void 0
            : _d.get(+elementId);
    ...
    previewNode(node) {
      ...
      return oneLine(
        `<${element.nodeName.toLowerCase()}${attrText}>${trimStringWithEllipsis(
          text,
          50
        )}</${element.nodeName.toLowerCase()}>`
      );
...
${i + 1}) ${info.preview} aka ${asLocator(this._sdkLanguage, info.selector)}`
      ...
      return this.createStacklessError(`strict mode violation: ${asLocator(
        this._sdkLanguage,
...
`);
    ...
    expectArray(elements, options) {
      ...
      const received = elements.map((e) =>
        options.useInnerText ? e.innerText : elementText(new Map(), e).full
      );
  ...
  window.InjectedScript = new (module.exports.InjectedScript())(
    globalThis,
    false,
    "undefined",
    "data-testid",
